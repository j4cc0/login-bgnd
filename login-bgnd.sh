...
if [ ! -s "$NEOFETCHPNG" ]; then
	logger -t "$SCRIPT" "[-] $NEOFETCHPNG has no useful result. Aborted"
	exit 1
fi
...
convert "$NEOFETCHPNG" -alpha on -channel a -evaluate multiply 0.25 +channel "$NEOFETCH"
if [ ! -s "$NEOFETCH" ]; then
	logger -t "$SCRIPT" "[-] $NEOFETCH has no useful result. Aborted"
	exit 1
fi
...
if [ ! -s "$BGND" ]; then
	logger -t "$SCRIPT" "[-] $BGND has no useful result. Aborted"
	exit 1
fi
...
if [ ! -s "$NEOFETCHSIZED" ]; then
	logger -t "$SCRIPT" "[-] $NEOFETCHSIZED has no useful result. Aborted"
	exit 1
fi
...
if [ ! -s "$BGNDLOGIN" ]; then
	logger -t "$SCRIPT" "[-] $BGNDLOGIN has no useful result. Aborted"
	exit 1
fi
